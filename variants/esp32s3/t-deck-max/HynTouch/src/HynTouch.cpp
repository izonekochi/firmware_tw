...
void hyn_touch_before_light_sleep()
{
    detach_irq_handler();
    g_touch.press_flag = false;
    // Put the CST3xx into its low-power state for the sleep: left in active-scan mode it draws
    // mA-class current the whole time the CPU sleeps. Touch/bezel are not wake sources on this
    // board (input is dropped while asleep anyway), and hyn_touch_after_light_sleep() already
    // calls tp_resum to restore scanning on wake.
    hyn_touch_suspend();
}
...
{
    g_touch.press_flag = false;
    // Light sleep can swallow a bezel-key release: a stale key_pressed[] latch would then
    // suppress the next press edge (edge filter in handle_key_report) and let the eventual
    // release pair with the pre-sleep press. Start every wake with clean key state.
    clear_key_state();
    hyn_touch_resume();
    g_touch.press_flag = false;
    attach_irq_handler();
